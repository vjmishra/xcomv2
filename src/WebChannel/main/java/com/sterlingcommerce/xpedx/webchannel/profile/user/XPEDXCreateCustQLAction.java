...
public class XPEDXCreateCustQLAction extends WCMashupAction {
	...
	public String execute() {
		...
		Object obj = WCMashupHelper.invokeMashup("manageCustomerQuickLink", input, wcContext.getSCUIContext());
	...
	private Element prepareInputXML () {
		...
		templateElement.setAttribute("OrganizationCode", getWCContext().getStorefrontId());
		Element eleCustomerContactList = templateCustomerDoc.createElement("CustomerContactList");
		Element eleCustomerContact = templateCustomerDoc.createElement("CustomerContact");
		...
		Element eleXPXQuickLinkList = templateCustomerDoc.createElement("XPXQuickLinkList");
		...
		if(!getFrmUserProfile1Flag().equalsIgnoreCase("true")){
			...
			while (st.hasMoreTokens()) {
				...
				if (i == 1) {
					eleXPXQuickLink = templateCustomerDoc.createElement("XPXQuickLink");
					String[] arrstr = token.split("%");
					String show = arrstr[0];
					eleXPXQuickLink.setAttribute("ShowQuickLink", show.trim());
					eleXPXQuickLink.setAttribute("QuickLinkName", arrstr[1].trim());
				}
