...
public class XPEDXCustomerBillTo extends WCMashupAction {
	...
	private static final Logger log = Logger.getLogger(XPEDXCustomerBillTo.class);
	...
	public String execute()
	{
		...
		try{
			...
			Document output = XPEDXWCUtils.
				getCustomerDetails(billToCustomerId, wcContext.getStorefrontId());
			...
			billToAddress = SCXmlUtil.getElementByAttribute(outputDoc,"CustomerAdditionalAddressList/CustomerAdditionalAddress", "IsDefaultBillTo", "Y");
			...
			String custCSR1UserId = SCXmlUtil.getAttribute(extnElem, "ExtnECSR");
			String custCSR2UserId = SCXmlUtil.getAttribute(extnElem, "ExtnECSR2");
			if(custCSR1UserId!=null && custCSR1UserId.trim().length()>0)
				csr1UserEle = XPEDXWCUtils.getUserInfo(custCSR1UserId, getWCContext().getStorefrontId());
			if(custCSR2UserId!=null && custCSR2UserId.trim().length()>0)
				csr2UserEle = XPEDXWCUtils.getUserInfo(custCSR2UserId, getWCContext().getStorefrontId());
