...
public class XPEDXGetShipToBillToInfoAction extends WCMashupAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1193316490821054386L;
	...
	public String execute() {
		...
		if (suffixType != null && !suffixType.trim().isEmpty()) {
			if (suffixType
	...
	private void setDisplayCustomerFormat() {
		...
		if (suffixType != null) {
			if (suffixType
				...
				/*
				 * if (billToAddress != null) { for (int index = 0; index < 6;
				 * index++) { if (billToAddress.getAttribute("AddressLine" +
				 * index) != null && billToAddress .getAttribute("AddressLine" +
				 * index) .trim().length() > 0) billtoAddr += ", " +
				 * billToAddress.getAttribute("AddressLine" + index); }
				 * 
				 * if (billToAddress.getAttribute("City") != null &&
				 * billToAddress.getAttribute("City").trim() .length() > 0)
				 * billtoAddr += ", " + billToAddress.getAttribute("City"); if
				 * (billToAddress.getAttribute("State") != null &&
				 * billToAddress.getAttribute("State").trim() .length() > 0)
				 * billtoAddr += ", " + billToAddress.getAttribute("State"); if
				 * (billToAddress.getAttribute("ZipCode") != null &&
				 * billToAddress.getAttribute("ZipCode").trim() .length() > 0)
				 * billtoAddr += " " + billToAddress.getAttribute("ZipCode"); if
				 * (billToAddress.getAttribute("Country") != null &&
				 * billToAddress.getAttribute("Country").trim() .length() > 0)
				 * billtoAddr += " " + billToAddress.getAttribute("Country");
				 * 
				 * }
				 */
				if (formattedCusotmer != null && orgName != null)
				...
				/*
				 * String localID = getLocationId(); if (localID != null &&
				 * !("").equals(locationId)) { shipToAddr += "Local ID: " +
				 * localID + " "; }
				 * 
				 * if (shipToAddress != null) { for (int index = 0; index < 6;
				 * index++) { if (shipToAddress.getAttribute("AddressLine" +
				 * index) != null && shipToAddress .getAttribute("AddressLine" +
				 * index) .trim().length() > 0) shipToAddr += ", " +
				 * shipToAddress.getAttribute("AddressLine" + index); } //
				 * if(formattedCusotmer!=null && orgName!=null) if
				 * (shipToAddress.getAttribute("City") != null &&
				 * shipToAddress.getAttribute("City").trim() .length() > 0)
				 * shipToAddr += ", " + shipToAddress.getAttribute("City"); if
				 * (shipToAddress.getAttribute("State") != null &&
				 * shipToAddress.getAttribute("State").trim() .length() > 0)
				 * shipToAddr += ", " + shipToAddress.getAttribute("State"); if
				 * (shipToAddress.getAttribute("ZipCode") != null &&
				 * shipToAddress.getAttribute("ZipCode").trim() .length() > 0)
				 * shipToAddr += " " + shipToAddress.getAttribute("ZipCode"); if
				 * (shipToAddress.getAttribute("Country") != null &&
				 * shipToAddress.getAttribute("Country").trim() .length() > 0)
				 * shipToAddr += " " + shipToAddress.getAttribute("Country"); }
				 */
