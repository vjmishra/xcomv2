...
public class XPEDXGetCustomerMaintenanceAction extends WCMashupAction {
	...
	protected void createEmpNames(){
		NodeList custContacts = empNameDoc.getElementsByTagName("CustomerContact");
		for (int i = 0; i < custContacts.getLength(); i++) {
				Element custContact = (Element)custContacts.item(i);
				Element custContactExtn = (Element)custContact.getElementsByTagName("Extn").item(0);
				String fname = custContact.getAttribute("FirstName");
				String lname = custContact.getAttribute("LastName");
				String name = lname + ", " + fname;
				if(PrimarySalesRepID.equals(custContactExtn.getAttribute("ExtnEmployeeID"))){
	...
	protected void createEmpList(){
		...
		Element salesRepList = XMLUtilities.getChildElementByName(extnElem, "XPEDXSalesRepList");
		...
		for (int i = 0; i < salesReps.getLength(); i++) {
			...
			if("Y".equalsIgnoreCase(salesRep.getAttribute("PrimarySalesRepFlag"))){
	...
	protected void setCustAddressToDisp(){
		Element custAddntlAddressList = XMLUtilities.getChildElementByName(outputDoc, "CustomerAdditionalAddressList");
		NodeList AddtnlAddressList = custAddntlAddressList.getElementsByTagName("CustomerAdditionalAddress");
	...
	protected void setCustMaintenanceFlags() {
		...
		Element extnElem = XMLUtilities.getChildElementByName(outputDoc, "Extn");
		//String vwPrices = SCXmlUtils.getAttribute(extnElem, "ExtnViewPricesFlag");
		//String cnOrder = SCXmlUtils.getAttribute(extnElem, "ExtnCanOrderFlag");
		//String vwInv = SCXmlUtils.getAttribute(extnElem, "ExtnCanViewInvFlag");
		//String uCustSKU = SCXmlUtils.getAttribute(extnElem, "ExtnUseCustSKUFlag");
		String uOMulUOM = SCXmlUtils.getAttribute(extnElem, "ExtnUseOrderMulUOMFlag");
		//String srFlag = SCXmlUtils.getAttribute(extnElem, "ExtnSampleRequestFlag");

		String eViewInvoices = SCXmlUtils.getAttribute(extnElem, "ExtnEmailViewInvoicesFlag");
		String pInvoices = SCXmlUtils.getAttribute(extnElem, "ExtnPreviewInvoicesFlag");
		String aBackOrders = SCXmlUtils.getAttribute(extnElem, "ExtnAcceptBackOrdersFlag");
		String aShipToOR = SCXmlUtils.getAttribute(extnElem, "ExtnShipToOverrideFlag");
		...
		if ("Y".equals(SCXmlUtils.getAttribute(extnElem, "ExtnMyItemsLink")))  myItemsLink = true;

		if ("Y".equals(SCXmlUtils.getAttribute(extnElem, "ExtnOrderUpdateFlag")))  orderUpdateFlag = true;
		if ("Y".equals(SCXmlUtils.getAttribute(extnElem, "ExtnShipComplete")))  shipComplete = true;
	...
	public boolean isCanViewInv() {
		return canViewInv;
	}

	public boolean isUseCustSKU() {
		return useCustSKU;
	}*/

	/*public boolean isUseOrderMulUOM() {
		return useOrderMulUOM;
	}*/

	/*public boolean isSampleRequest() {
		return sampleRequestFlag;
	}*/

	/*public boolean isEmailViewInvoices() {
		return emailViewInvoices;
	}
	...
	public boolean isAcceptBackOrders() {
	...
	private static final Logger log = Logger.getLogger(XPEDXGetCustomerMaintenanceAction.class);
