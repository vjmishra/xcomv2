...
public class XPEDXGetDivisionArticle extends WCMashupAction {
	...
	public void updateLastLoginDate() {
		...
		/****
		 * Added for Jira xb 621 Below code is used to check from table
		 * xpxCustomercontactextn to see if the user has accepted the terms of
		 * access and if yes it would update the recently logged in date to the
		 * Database.
		 */
		...
		//	String lastLoginDate = (String) XPEDXWCUtils.getObjectFromCache("LastLoginDate");//removed as part of EB-475
			...
			if (xpxCustContExtnEle == null) {
				createCCExtn = true;// added if the customer contact has to be
			...
			attributeMap.put(
					XPEDXConstants.XPX_CUSTCONTACT_EXTN_LAST_LOGIN_DATE,
					lastLoginDate);
