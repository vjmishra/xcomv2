...
public class XPEDXGetCustomerQuickLinkAction extends WCMashupAction {
	...
	public String execute() {
		...
		try {
			...
			/*boolean isSAPCustomeInSession=false;
			if(customerId == null || customerId.trim().length() == 0) {
				
				if(billToCustomerObj != null)
				{
					XPEDXShipToCustomer sapCustomerObj=billToCustomerObj.getBillTo();
					if(sapCustomerObj != null)
					{
						customerId=sapCustomerObj.getCustomerID();
						if(customerId != null)
							isSAPCustomeInSession=true;
					}
				}
				if(isSAPCustomeInSession ==false) {
					
					Document billToCustomer = XPEDXWCUtils.getCustomerDocument(billToCustomerObj.getCustomerID(), wcContext.getStorefrontId());
					if(billToCustomer!=null)
					{
						...
						if(sapCustElement!=null)
							customerId = sapCustElement.getAttribute("CustomerID");// SAPcustomerID
					}
					...
					Document SAPCustomerDoc = XPEDXOrderUtils.getCustomerExtnFlagsDoc(wcContext);
					if(SAPCustomerDoc!=null)
						customerId = SAPCustomerDoc.getDocumentElement().getAttribute("CustomerID");
				}
			}*/
	...
	private QuickLinkBean[] getQuickLink(Element outputElement)throws RuntimeException {
		...
		if(outputElement!=null) {
			Node quickLinkNodeList = outputElement.getFirstChild().getFirstChild().getFirstChild().getFirstChild();
			...
			for (int i=0; i<quickLinkNode.getLength(); i++) {
				...
				quickLinkBean[i].setUrlName(quickLinkElement.getAttribute("CustQuickLinkName"));
				quickLinkBean[i].setQuickLinkURL(quickLinkElement.getAttribute("CustQuickLinkUrl"));
				quickLinkBean[i].setUrlOrder(Integer.parseInt(quickLinkElement.getAttribute("CustURLOrder")));
				quickLinkBean[i].setShowQuickLink(quickLinkElement.getAttribute("CustShowQuickLink"));
			}
