...
public class XPEDXAssignedCustomersLocationsDisplayAction extends
	...
	public String execute() {
		/*Picking the default shipTo from the user profile and getting the ship to Address directly from the jsp
		 This change is done for performance increase and user profile clean up
		 */
...
//		}
	...
	private void getDefaultShipToAddress() {
		...
		if (defaultShipTo != null) {
			...
			log.debug("No Default Ship To is selected");
		}
	...
	private void getCustomerAssignments(){
		try{
			assignedCustomersMap = XPEDXWCUtils
					.getAssignedCustomerElementHashMap(XPEDXWCUtils.getLoggedInCustomerFromSession(wcContext),loggedInUserId);
			...
			if(billToList!=null) {
				for(int i=0;i<billToList.size();i++) {
					if(billToListText!= null)
						billToListText = billToListText+","+billToList.get(i);
					else
						billToListText = billToList.get(i)+",";
				}
			}
//			customerAssignmentElem = prepareAndInvokeMashup("xpedx-getCustomerAssignments");
	 ...
	 * so that, we get only the required information - here ParentInformation.
	 * @param inputItems
	 * @return
	 */
	private void getCustomerAssignmentsAsHierarchy() {
		...
		try {
			...
			Element customerEle = XPEDXWCUtils.getCustomerDetails(customerId,
					wcContext.getStorefrontId(), parentCustomerInformationMashUp).getDocumentElement();
			...
			NodeList billToNL = SCXmlUtil.getXpathNodes(outputDoc,
					"/BillToList/BillTo");
			prepareHierarchyToDisplay(billToNL);
		} catch (Exception e) {
			log
					.debug("Error while getting the Bill to and Ship to Hierarchy ... ");
			e.printStackTrace();
		}
	...
	private void prepareHierarchyToDisplay(NodeList billToNL) {
		customerHierachyDoc = YFCDocument.createDocument("CustomerAssignment")
				.getDocument();
		Element customerHierachyElem = customerHierachyDoc.getDocumentElement();
		for (int i = 0; i < billToNL.getLength(); i++) {
			Element billToElem = (Element) billToNL.item(i);
			String billToID = billToElem.getAttribute("BillToID");
			if (assignedCustomerIDs.contains(billToID)) {
				Element billToCustElem = SCXmlUtil.getElementByAttribute(
						customerAssignmentElem, "CustomerAssignment/Customer",
						"CustomerID", billToID);
				...
				Element assignedbillToShipToElem = customerHierachyDoc
						.createElement("ShipToList");
				List<String> shipToIDsList = new ArrayList<String>();
				for (int j = 0; j < shipToListOfBillTO.getLength(); j++) {
						...
						if (shipToIDsList.size() == 0) {
							...
							Element ShipToCustElem = SCXmlUtil
									.getElementByAttribute(
											...
											"CustomerID", shipToID);
							assignedbillToShipToElem
									.appendChild(ShipToCustElem);
							assignedbillToElem
					...
					if (assignedCustomerIDs.contains(shipToID.trim())) {
						if (shipToIDsList.size() == 0) {
							...
							try {
								...
								e.printStackTrace();
								log
										.debug("Error while getting the Bill To customer Info");
							}
							...
							assignedbillToElem.appendChild(billToofShipToElem);
							Element ShipToCustElem = SCXmlUtil
									.getElementByAttribute(
											customerAssignmentElem,
											"CustomerAssignment/Customer",
									...
									.setAttribute(
											...
											SCXmlUtil
													.getXpathAttribute(
															...
															"/Customer/Extn/@ExtnCustomerName"));
							assignedbillToShipToElem
									.appendChild(hierarchyShipToElem);
							assignedbillToElem
									.appendChild(assignedbillToShipToElem);
						} else {
							shipToIDsList.add(shipToID);
							Element ShipToCustElem = SCXmlUtil
									.getElementByAttribute(
											...
											"CustomerAssignment/Customer",
											"CustomerID", shipToID);
							if (ShipToCustElem == null) {
								try {
									ShipToCustElem = XPEDXWCUtils
									...
									e.printStackTrace();
									log
											.debug("Error while getting the Ship To customer Info");
								}
							}
							Element hierarchyShipToElem = customerHierachyDoc
									.createElement("Customer");
							hierarchyShipToElem.setAttribute(
									"ExtnCustomerName", SCXmlUtil
											.getXpathAttribute(ShipToCustElem,
													"Extn/@ExtnCustomerName"));
							assignedbillToShipToElem
									.appendChild(hierarchyShipToElem);
							assignedbillToElem
									.appendChild(assignedbillToShipToElem);
	...
	public String getBillToParentCustomerKey() {
		return billToParentCustomerKey;
	}
	...
	public Document getCustomerHierachyDoc() {
		return customerHierachyDoc;
	}
