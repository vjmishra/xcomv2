...
public class XPEDXSaveCustomerInfo extends WCMashupAction {
		...
		public String execute() {
			...
			Object obj = WCMashupHelper.invokeMashup("xpedx-saveCustomerQuickLinks", input, wcContext.getSCUIContext());
		...
		private Element prepareInputXMLforCustQuickLinks() {
			...
			Document templateCustomerDoc = YFCDocument.createDocument("Customer").getDocument() ;		
			...
			Element eleXPXQuickLinkList = templateCustomerDoc.createElement("XPXCustQuickLinkList");
			eleXPXQuickLinkList.setAttribute("Reset", "true");
			
			Element addtnlAddressList = templateCustomerDoc.createElement("CustomerAdditionalAddressList");
			Element addtnlAddress = templateCustomerDoc.createElement("CustomerAdditionalAddress");
			...
			if((addtnlAddressID!=null || addtnlAddressKey!=null)&&(addtnlAddressID.trim().length()>0 || addtnlAddressKey.trim().length()>0)) {
				addtnlAddress.setAttribute("CustomerAdditionalAddressID",addtnlAddressID);
				addtnlAddress.setAttribute("CustomerAdditionalAddressKey",addtnlAddressKey);
...
//			changed to work only when delimiters appear in sequence
			...
			//while (st.hasMoreTokens()) 
			{
				...
				if (i == 1) {
					eleXPXQuickLink = templateCustomerDoc.createElement("XPXCustQuickLink");
