...
<script type="text/javascript"
	src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/js/webtrends/displayWebTag<s:property value='#wcUtil.xpedxBuildKey' />.js"></script>
<!-- Web Trends tag end  -->
<script type="text/javascript"
	src="<s:property value='#wcUtil.staticFileLocation' />/swc/js/common/XPEDXUtils<s:property value='#wcUtil.xpedxBuildKey' />.js"></script>
<script type="text/javascript"
	src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/js/jquery-ui-1/development-bundle/ui/jquery.ui.tabs<s:property value='#wcUtil.xpedxBuildKey' />.js"></script>
...
<link rel="stylesheet" type="text/css"
	...
	media="screen" />
<script type="text/javascript"
	src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/js/modals/checkboxtree/jquery.checkboxtree<s:property value='#wcUtil.xpedxBuildKey' />.js"></script>
<script type="text/javascript"
	src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/js/sorttable<s:property value='#wcUtil.xpedxBuildKey' />.js"></script>
<script
	src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/js/SpryTabbedPanels<s:property value='#wcUtil.xpedxBuildKey' />.js"
	...
	document.getElementById("strdiv"+val1).style.display="block";
		...
		if(!salesRep){
			...
			if(submitOrderChkValue){
				...
				if((primaryApprover == "" || primaryApprover == null)){
					document.getElementById("errorMsgFor_spendingLtSubmitOrder").style.display = "none";
...
div table#myTable td {
	padding: 5px;
	word-wrap: break-word;
	border-bottom: 1px solid #CCCCCC;
}

div table#myTable td.left-cell {
	border-left: 0px;
}

div table#myTable td.right-cell {
	border-right: 0px;
}

div table#myTable td.bottom-cell {
	border-bottom: 0px;
}

div table#myTable td div {
	width: 179px;
}

div table#myTable td input {
	margin-right: 5px;
}
...
<link
	href="<s:property value='#wcUtil.staticFileLocation' />/xpedx/css/theme/SpryTabbedPanels<s:property value='#wcUtil.xpedxBuildKey' />.css"
...
<body class="ext-gecko ext-gecko3">
	...
	<s:if test='%{#userExists}'>
		...
		<s:set name='maskedPasswordString'
			value='%{getMaskedPasswordString()}' />
	...
	<div id="main-container">
								...
								<td colspan="2" class="no-border-right-user"><s:if
										...
										<s:else>
											<s:textfield tabindex="5" disabled="%{#isDisabled}"
												name="userName" cssClass="x-input" cssStyle="width: 130px;" />
		...
		<s:set name="errorNote" value="<s:property value=null />" scope="session"/>
							...
							<s:if test='%{#isCustomerNotAdmin == false}'>
								<tr>
										...
										<div class="question">
											<ul>
												<s:form name="userList" id="userList">
													<li><a id="selectusertomodifyProfile"
														href="#viewUsersDlg" class="underlink">Show My
															Users&nbsp;</a></li>
													<li><a href="#"><img
															src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/images/icons/12x12_grey_help.png"
															style="margin-top: 2px;" border="0" width="12"
															height="12"
															alt="Displays all users set up for this account."
															title="Displays all users set up for this account."
															align="right" /></a></li>
												</s:form>
											</ul>
										</div>
									...
									<td width="36%" align="left"
										class="padding-left0 no-border-right-user">
										<div class="question">
											<ul>
												<s:set name='custID' value='#customerId' />
												<s:set name='storeFrontID' value="wCContext.storefrontId" />
												<li><s:form name="userListaddForm" id="userListaddForm">
														<%--JIRA 3917 Finish --%>
														<s:hidden name="customerID"
															value="%{#_action.getCustomerID()}" />
														<s:hidden name="buyerOrgCode" value="%{#buyerOrgCode}" />
														<s:url id="showAllUsersURL" action='getUserList'
															namespace='/profile/user'>
														...
														<s:set name='custDoc'
															value="@com.sterlingcommerce.xpedx.webchannel.utilities.XPEDXWCUtils@getCustomerDetails(#custID,#storeFrontID)" />
														<s:set name='userCustElement'
															value='#custDoc.getDocumentElement()' />
														<s:set name='extnSuffixType'
															value='%{#SCXmlUtils.getXpathAttribute(#userCustElement, "//Extn/@ExtnSuffixType")}' />
														<div id="searchTable">
					...
					<s:form id="myAccount" name="myAccount" method="post"
						validate="true" action="saveUserProfileInfo">
						...
						<s:hidden name="POListText" id="POListText" value='%{#poListVal}' />
						...
						<s:hidden name='userExistsId' value='%{#userExists}'
							id='userExistsId' />
						<s:hidden name='selfAdminId' id="selfAdminId"
							value="%{isSelfAdmin()}" />
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												<td valign="top" colspan="2"
													...
													<div class="question">
														<ul>
															<li><span class="subtitle">Catalog
																	Preferences</span></li>
														</ul>
													</div>
												</td>
												...
												<td width="27%" valign="top"
													...
													<div class="question">
														<ul>
															...
															<li><a href="#"><img
																	src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/images/icons/12x12_grey_help.png"
																	style="margin-top: 2px;" width="12" height="12"
																	border="0"
													...
													<div class="question">
														<ul>
															...
															<li><a href="#"><img
																	...
																	title="Defaults your preferred catalog on the homepage."
																	width="12" height="12" border="0" /></a></li>
														</ul>
													</div>
												</td>
												<td valign="top"
													class="no-border-right-user user-profile padding0 "><s:select
														headerKey="" headerValue="- Select Category -"
														name="PrefCatalog" value='prefCategory'
											...
											<tr>
												...
												<td valign="top" class="no-border-right-user padding0"
													...
													<table border="0" align="left" cellpadding="0"
														cellspacing="0" id="mil-list-new"
														style="background: none repeat scroll 0% 0% transparent; width: 80%;">
														<tbody>
															<tr>
																<td width="55%" valign="top"
																	class="noBorder noBorders-FAFAFA"><select
																	style="width: 250px;" id="selectPOName" name="select"
																	multiple="multiple">
																		<s:iterator value="POList" id="poListTbl"
																			status="poListStatus">
																			<s:set name="poListValue" value='value' />
																			<option>
																				<s:property value="poListValue" />
																			</option>
																		</s:iterator>
																</select></td>
																<td valign="top" class="noBorder">
																	...
																	class="input-details-cart  x-input"
																	style="width: 150px;" title="POname" tabindex="12"
																	maxlength="22" />
											...
											<tr>
												...
												<td valign="top" class="no-border-right-user padding0"><span
													class="clearview"> Additional Order Confirmation
														Email Address(es):</span>
													...
													<table border="0" align="left" cellpadding="0"
														...
														<tbody>
															<tr>
																<td width="55%" valign="top"
																	class="noBorder noBorders-FAFAFA"><select
																	style="width: 250px;" name="selectOrderConfEmails"
																	id="selectOrderConfEmails" multiple="multiple">
																		<s:iterator value="AddnlEmailAddrList"
																			id="AddnlEmailAddrListTbl"
																			status="AddnlEmailAddrListStatus">
																			<s:set name="AddnlEmailAddr" value='value' />
																			<option>
																				<s:property value="#AddnlEmailAddr" />
																			</option>
																		</s:iterator>
																</select></td>
																<td valign="top" class="noBorder">
																	<div class="add-list-label">Enter value to add:</div> <input
																	name="confEmailText" id="confEmailText"
																	class="input-details-cart  x-input" id="Text4"
			...
			<td valign="top" class="no-border-right-user padding00">&nbsp;</td>
							...
							<s:else>
								<ul class="TabbedPanelsTabGroup" style="margin-left: 5px;">
									...
									<s:if
										test="%{(#optedCurrency != null && #optedCurrency!='') || ( #primaryApprover != null && #primaryApprover !='' ) || ( #spendingLimitValue != null && #spendingLimitValue !='') || (#alternateApprover != null && #spendingLimitValue !='')}">
								...
								<div id="TabbedPanels1" class="TabbedPanelsContentGroup">
									<div id="UserInformationsTab" class="TabbedPanelsContent">
										<div>
											...
											<table width="100%" border="0" cellspacing="0"
												cellpadding="0" class="tabs line-spacing-tr">
												...
												<s:if test='%{#isCustomerNotAdmin == false}'>
													<tr id="userNameRow">
														...
														<td colspan="3" class="no-border-right-user"><s:iterator
																value="getStatusList().keySet()" id="statusVal">
																<s:set name="statusValue" value="value" />
																<label> <%-- Fix for Jira 3048 --%> <s:if
																		test="%{#disableSinceSelfApprover}">
																		<input tabindex="<s:property value="#tabidx"/>"
																			type="radio" name="status" id='status'
																			disabled='true'
																			value='<s:property value="statusVal"/>'
																			<s:if test ="%{#statusVal == getContactStatus()}" > checked="true"</s:if>>
																		</input>
																		<s:if test="%{#statusVal == getContactStatus()}">
																			<s:hidden name="status" id='status'
																				value='%{getContactStatus()}'></s:hidden>
																		</s:if>
																	</s:if> <s:else>
																		<input tabindex="<s:property value="#tabidx"/>"
																			type="radio" name="status" id='status'
																	...
																	</s:else> <%-- END Fix for Jira 3048 --%> <s:if
																		test="%{#statusVal == 30}">
					  Suspended
					</s:if> <s:else>
																		<s:property value="%{getStatusList().get(#statusVal)}" />
																	</s:else> <s:set name="tabidx" value="%{#tabidx +1}" />
																</label>
															</s:iterator>
															<div class="float-right">
																<span>Last Login: <s:property
																		value="%{getExtnLastLoginDate()}" />
																</span><span class="padding-left2">User Created: <s:property
																		value="%{getUserCreatedDate()}" />
																</span>
															</div></td>
													</tr>
												...
												<s:else>
													...
													<tr id="userNameRow">
														...
														<td colspan="3" class="no-border-right-user"><s:if
																test="#userStatus == '10'">
                   Active
            </s:if> <s:if test="#userStatus == '20'">
                 On Hold
            </s:if> <s:if test="#userStatus == '30'">
                 Inactive
             </s:if>
															<div class="float-right">
																<span>Last Login: <s:property
			 ...
			 End Fix for Jira 3048 item issue 1 --%>
														<td colspan="3" class="no-border-right-user"><s:if
																...
																<label
																	...
																	<s:checkbox tabindex="11" name='buyerAdmin'
																		id='buyerAdmin' fieldValue="true"
																		value='%{isInUserGroup("BUYER-ADMIN")}' /> Admin
																</label>
																<label title="Authorizes submission of orders.">
																	<s:checkbox tabindex="13" name='buyerApprover'
																		id='buyerApprover'
																		value='%{isInUserGroup("BUYER-APPROVER")}'
																		fieldValue="true" /> Approver
																</label>
																<label title="Buyer has the ability to submit orders.">
																	<s:checkbox tabindex="14" name='test' id='test'
																		disabled="true" fieldValue="test123"
																		value="%{isInUserGroup('BUYER-USER')}" />Buyer
																</label>
															</s:if> <s:else>
																<label
																	title="Responsible for overall administration of, and access to, accounts on the web site. Creates user profiles, assigns roles, assigns locations.">
																	<s:checkbox tabindex="11" name='buyerAdmin'
																		id='buyerAdmin' fieldValue="true"
																		value='%{isInUserGroup("BUYER-ADMIN")}'
																		disabled="true" /> Admin
																</label>
																<s:hidden name="buyerAdmin" id='buyerAdmin'
																	value='%{isInUserGroup("BUYER-ADMIN")}'></s:hidden>
																<label title="Authorizes submission of orders.">
																	<s:checkbox tabindex="13" name='buyerApprover'
																		id='buyerApprover'
																		value='%{isInUserGroup("BUYER-APPROVER")}'
																		fieldValue="true" disabled="true" /> Approver
																</label>
																...
																<label title="Buyer has the ability to submit orders.">
																	<s:checkbox tabindex="14" name='test' id='test'
																		fieldValue="test123"
																		value="%{isInUserGroup('BUYER-USER')}" disabled="true" />Buyer
																</label>
															...
															</s:else> <s:if test='%{#viewInvoices=="Y"}'>
																<label title="Permitted to view invoices online.">
																	<s:checkbox tabindex="16" name="viewInvoices"
																		fieldValue="true" id="viewInvoices" value="true"
																		disabled='%{#checkBoxDisable || #isDisabled}' />
																...
																<label title="Permitted to view invoices online.">
																	<s:checkbox tabindex="16" name="viewInvoices"
																		fieldValue="true" id="viewInvoices" value="false"
																		disabled='%{#checkBoxDisable || #isDisabled}' />
																	<p class="float-right">View Invoices</p>
																</label>
															</s:else> <label title="Permitted to view prices."> <s:checkbox
																	tabindex="17" fieldValue="true" name="viewPrices"
																	id="viewPrices" value="%{isViewPrices()}"
																	disabled='%{#checkBoxDisable || #isDisabled}' /> View
																Prices
														</label> <label title="Permitted to view reports."> <s:checkbox
																	tabindex="18" name="viewReports" id="viewReports"
																	fieldValue="true" value='%{isViewReports()}'
																	disabled='%{#checkBoxDisable || #isDisabled}' />
																<p class="float-right">View Reports</p></label></td>
													</s:if>
													<s:else>
														<td colspan="3" class="no-border-right-user"><s:if
			...
			<s:hidden name='viewReports' value='%{true}' />
												...
												<tr>
													<td class="no-border-right-user">
														<div class="mandatory float-left">*</div> First Name:
													</td>
													<td width="31%" class="no-border-right-user"><s:textfield
															tabindex="20" name="firstName" id="firstName"
															onkeyup="javascript:testFieldValueCheck(this, 'myAccount');"
															onmouseover="javascript:testFieldValueCheck(this, 'myAccount');"
															maxlength="20" cssClass="x-input"
													...
													<td width="19%" class="no-border-right-user">
														<div class="mandatory float-left">*</div> Last Name:
													</td>
													<td width="37%" class="no-border-right-user"><s:textfield
															tabindex="21" name="lastName" id="lastName"
															onkeyup="javascript:testFieldValueCheck(this, 'myAccount');"
															onmouseover="javascript:testFieldValueCheck(this, 'myAccount');"
												...
												<s:if test="%{selfAdmin}">
													...
													<tr>
														...
														<td width="31%" class="no-border-right-user"><s:url
																id="ValidatePasswordURL" action="validatePassword" /> <s:password
														...
														<td width="37%" class="no-border-right-user"><s:password
																tabindex="23" name="confirmpassword"
													...
													<s:if test="#orgQuestionList!=null">
														<tr>
															<td class="no-border-right-user">
																<div class="mandatory float-left">*</div> Security
																Question:
															</td>
															<td colspan="3" class="no-border-right-user"><s:select
																	headerKey="" headerValue="- Security Question -"
																	tabindex="24" name="secretQuestion" id="secretQuestion"
																	value="%{#userSecretQuestion}" list="#orgQuestionList"
																	cssClass="x-input" cssStyle="width: auto;"
													...
													<tr>
														...
														<td class="no-border-right-user"><s:password
																tabindex="25" name='secretAnswer' id="secretAnswer"
																onkeyup="javascript:testFieldValueCheck(this, 'myAccount');"
																onmouseover="javascript:testFieldValueCheck(this, 'myAccount');"
																cssClass="x-input" cssStyle="width: 185px;"
																value="%{#maskedAnswerString}" showPassword="true" /></td>
														<td class="no-border-right-user">
															<div class="mandatory float-left">*</div> Confirm Answer:
														</td>
														<td class="no-border-right-user"><s:password
																tabindex="26" name='confirmAnswer' id="confirmAnswer"
																onkeyup="" cssClass="x-input" cssStyle="width: 185px;"
																value="%{#maskedAnswerString}" showPassword="true" /></td>
													</tr>
												...
												<tr>
													<td valign="top" class="no-border-right-user">
														<div class="question">
															<ul>
																<li>
																	<div class="mandatory float-left">*</div> Email
																	Address:&nbsp;
																</li>
																<li><a href="#"><img
																		src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/images/icons/12x12_grey_help.png"
													...
													<td class="no-border-right-user"><s:textfield
															tabindex="27" maxlength="500" name="emailId"
															cssClass="x-input" id="emailId"
															onkeyup="javascript:testFieldValueCheck(this, 'myAccount');"
															onmouseover="javascript:testFieldValueCheck(this, 'myAccount');"
															cssStyle="width: 185px;" disabled="%{#isDisabled}"
															value='%{#customercontact.getAttribute("EmailID")}' /></td>
													<td class="no-border-right-user">
														<div class="mandatory float-left">*</div> Confirm Email
														Address:
													</td>
													<td class="no-border-right-user"><s:textfield
															tabindex="28" maxlength="500" id="confirmEmailId"
															name="confirmEmailId" onkeyup="" cssClass="x-input"
															cssStyle="width: 185px;" disabled="%{#isDisabled}"
															value='%{#customercontact.getAttribute("EmailID")}'
															onchange="javascript:return confirmEmail();" /></td>
		...
		<tr><td valign="top" colspan="4" width="100%" class="no-border-right-user"><div class="error" id="errorMsgFor_validemailId" style="display : none"/>Please enter a valid email address.</div></td></tr> -->
												...
												<s:if
													test='%{#hasAccessToEdit != "TRUE" || #listEmty == null}'>
												...
												<tr>
													...
													<td class="no-border-right-user"><s:textfield
															name="state_new" tabindex="33"
															value="%{#personInfo.getAttribute('State')}"
															id="state_new" cssClass="x-input"
															cssStyle="width: 190px;" maxlength="2" /></td>
												</tr>
												<tr>
													<td class="no-border-right-user">Postal Code:</td>
													<td class="no-border-right-user"><s:textfield
															name='zipCode_new' tabindex="34"
															value='%{#personInfo.getAttribute("ZipCode")}'
															id='zipCode_new' cssClass="x-input"
															cssStyle="width: 190px;" maxlength="10" /></td>
													<td class="no-border-right-user">Country:</td>
													<td class="no-border-right-user"><s:select
															tabindex="35" name="country_new" id="country_new"
															emptyOption="true"
															value='%{#personInfo.getAttribute("Country")}'
															list="%{getCountriesMap()}" headerKey=""
															headerValue="-Select Country-" cssClass="x-input"
															cssStyle="width: 190px;" /></td>
												</tr>
												<tr>
													<td class="no-border-right-user">Phone:</td>
													<td class="no-border-right-user"><s:textfield
															tabindex="36" name='dayPhone_new'
															value='%{#personInfo.getAttribute("DayPhone")}'
															id='dayPhone_new' cssClass="x-input phone-numeric"
															cssStyle="width: 190px;" maxlength="10" />
														<div class="error" id="errorMsgFor_validPhoneNo"
															style="display: none">Please enter a valid 10 digit
															number.</div></td>
													<td class="no-border-right-user">Fax:</td>
													<td class="no-border-right-user"><s:textfield
															tabindex="37" name='dayFaxNo_new'
															value='%{#personInfo.getAttribute("DayFaxNo")}'
															id='dayFaxNo_new' cssClass="x-input phone-numeric"
															cssStyle="width: 190px;" maxlength="10" />
														<div class="error" id="errorMsgFor_validFaxNo"
															style="display: none">Please enter a valid 10 digit
															number.</div></td>
												</tr>
											</table>
															...
															<div class="question">
																<ul>
																	...
																	<li><span class="float-left"><a href="#"><img
																				...
																				title="Preferred Authorized Location." width="12"
																				height="12" border="0" /></a></span></li>
																</ul>
															</div>
													...
													<s:if test="#shipTo != null">
														<td width="81%" valign="top"
															...
															<s:if test="%{#shipTo.getOrganizationName()!=''}">
																<s:property value='#shipTo.getOrganizationName()' />
																<br />
															</s:if> <s:iterator value="#shipTo.getAddressList()"
																id="adressLine">
																<s:property value="adressLine" />
																<br>
																<!-- break for user address line 2 and  jira 3244--->
															</s:iterator> <s:property value="#shipTo.getCity()" />, <s:property
		...
		<tr>
			...
			<td valign="top" class="no-border-right-user padding00">&nbsp;</td>
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0"
											...
											<tr>
												<td valign="top" colspan="4" width="100%"
													class="no-border-right-user"><div class="error"
														id="errorMsgFor_approvers" style="display: none" />Please
													select an approver.
													</div></td>
											</tr>
											<tr>
												<td valign="top" colspan="4" width="100%"
													class="no-border-right-user"><div class="error"
														id="errorMsgFor_currency" style="display: none" />Please
													select a currency.
													</div></td>
											</tr>
											<tr>
												<td valign="top" colspan="4" width="100%"
													class="no-border-right-user"><div class="error"
														id="errorMsgFor_spendingLtSubmitOrder"
														style="display: none" />All orders will be submitted for
													approval. Remove the spending limit and try again.
													</div></td>
											</tr>
											<tr>
												<td valign="top" colspan="4" width="100%"
													class="no-border-right-user"><div class="error"
														id="errorMsgFor_approverSubmitOrder" style="display: none" />Please
													select a Primary Approver.
													</div></td>
											</tr>
											...
											<tr>
												<td width="100%" colspan="2" valign="top"
													...
													<div style="width: 120px;">
														<span class="subtitle">Spending Limit&nbsp;</span> <a
															href="#"><img
															src="<s:property value='#wcUtil.staticFileLocation' />/xpedx/images/icons/12x12_grey_help.png"
															style="margin-top: 2px; display: inline;"
													...
													class="no-border-right-user padding00"><s:if
														...
														<s:else>
															<s:select headerKey="" headerValue="- Select Currency -"
																tabindex="20" name="spendingLtCurrency"
																id="spendingLtCurrency" value="%{#optedCurrency}"
																list="currencyMap" cssClass="x-input"
											...
											<tr>
												<td width="16%" valign="top"
													class="no-border-right-user padding00">Spending Limit:</td>
												<td valign="top" width="84%"
													class="no-border-right-user padding00"><s:set
														name='spendingLimit' value="%{showSpendingLimit()}" /> <s:if
														...
														<s:else>
															<s:textfield cssClass="x-input" cssStyle="width:148px;"
																id="spendingLt" name="spendingLt" maxlength="6"
																value="%{getUnformattedSpendingLimit()}"></s:textfield>
														</s:else>
													</s:if> <s:else>
														<s:if test="!#disableSinceSelfApprover">
															<s:textfield cssClass="x-input" cssStyle="width:148px;"
																id="spendingLt" name="spendingLt" maxlength="6" value=""></s:textfield>
														</s:if>
													</s:else></td>
											</tr>
											<tr>
												<td width="19%" valign="top"
													class="no-border-right-user padding00">Submit All
													Orders For Approval:</td>
											...
											<tr>
												<td width="100%" colspan="2" valign="top"
													class="no-border-right-user padding00"><span
													class="subtitle">Approvers</span></td>
											</tr>
											<tr>
												<td width="16%" valign="top"
													class="no-border-right-user padding00">Primary
													Approver:</td>
												<td valign="top" width="84%"
													class="no-border-right-user padding00 "><s:if
														...
														<s:else>
															<s:select headerKey="" headerValue="- Select Approver -"
																tabindex="20" name="primaryApprover"
																id="primaryApprover" value="%{#primaryApprover}"
																list="availableApproversList" cssClass="x-input"
																cssStyle="width: 150px;">
															</s:select>
														</s:else>
													</s:if> <s:else>
														<s:if test="!#disableSinceSelfApprover">
															<s:select headerKey="" headerValue="- Select Approver -"
																tabindex="20" name="primaryApprover"
																id="primaryApprover" value="- Select Approver -"
																list="availableApproversList" cssClass="x-input"
																cssStyle="width: 150px;">
															</s:select>
														</s:if>
											...
											<tr>
												<td valign="top" width="16%"
													class="no-border-right-user padding00">Alternate
													Approver:</td>
												<td valign="top" width="84%"
													class="no-border-right-user padding00"><s:if
														test="#alternateApprover != null && #alternateApprover != ''">
														<s:if test="#disableSinceSelfApprover">
														...
														<s:else>
															<span class="no-border-right-user padding0 "> <s:select
																	...
																	tabindex="20" name="alternateApprover"
																	id="alternateApprover" value="%{#alternateApprover}"
																	list="availableApproversList" cssClass="x-input"
																	cssStyle="width:150px;">
																</s:select>
															</span>
														...
														<s:if test="!#disableSinceSelfApprover">
															<span class="no-border-right-user padding0 "> <s:select
																	headerKey="" headerValue="- Select Approver -"
																	tabindex="20" name="alternateApprover"
																	id="alternateApprover" value="- Select Approver -"
																	list="availableApproversList" cssClass="x-input"
																	cssStyle="width:150px;">
																</s:select>
															</span>
							...
							<div class="success" id="msgFor_resetpassword"
								style="display: none; float: right" />
							Instructions to reset this password have been sent to the
							registered user's email address.
						</div>

						<div class="error" id="errorMsgFor_userpassword"
							style="display: none; float: right" />Please enter the same password in both Password and Confirm Password fields.
				
				</div>

				<div class="error" id="errorMsgForMandatoryFields_myAccount"
...
--%>
				...
				<s:if test="%{#session.errorNote!= null}">
					...
					<s:set name="errorNote" value="<s:property value=null />"
						scope="session" />

				</s:if>
				<%--End XB - 319 --%>

				<div class="error" id="errorMsgFor_emailId"
					style="display: none; float: right" />
				Please enter the same email address in both Email Address and
				Confirm Email Address fields.
			</div>

			<div class="error" id="errorMsgFor_secretAnswer"
				style="display: none; float: right" />
			Please enter the same Answer in both the Security Answer and the
			Confirm Answer fields.
		</div>

		<s:hidden name='test_success' id="test_success"
			value="%{#_action.isSuccess()}" />
...
<s:if test="%{!#isSalesRep}">
  ...
  function insertInLineFields()
  {
	...
	<div style="display: none;" class='x-hidden dialog-body '>
		<div id="viewShipToDlg"
			style="width: 745px; height: 397px; overflow: -moz-scrollbars-vertical;">
		</div>
	</div>
	<script type="text/javascript">
