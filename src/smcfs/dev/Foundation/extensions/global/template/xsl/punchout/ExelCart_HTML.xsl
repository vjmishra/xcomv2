...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="/">
		<xsl:for-each select="//OrderLine">
			...
			<input type="hidden" name="NEW_ITEM-VENDOR[{@PrimeLineNo}]" value="228343"/>
			<input type="hidden" name="NEW_ITEM-VENDORMAT[{@PrimeLineNo}]" value="{Item/@ItemID}"/>
			<input type="hidden" name="NEW_ITEM-CUSTOMER_SKU[{@PrimeLineNo}]" value="{CPN}"></input>
			<input type="hidden" name="NEW_ITEM-MANUFACTURER_SKU[{@PrimeLineNo}]" value="{Item/@ManufacturerItem}"></input>
			...
			<input type="hidden" name="NEW_ITEM-LONGTEXT_{@PrimeLineNo}:132[]" value="{ItemDetails/PrimaryInformation/@ExtendedDescription}"/>
			...
			<input type="hidden" name="NEW_ITEM-CUST_FIELD1[{@PrimeLineNo}]" value="{ItemDetails/Extn/@ExtnUNSPSC}"/>
		</xsl:for-each>
	</xsl:template>
</xsl:stylesheet>
