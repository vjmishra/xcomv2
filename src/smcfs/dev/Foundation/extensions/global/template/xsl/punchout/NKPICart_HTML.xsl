	...
	<xsl:template match="/">
		<FORM name="Form1">
			...
			<xsl:for-each select="//OrderLine">
				...
				<input type="hidden" name="NEW_ITEM-MATNR[{@PrimeLineNo}]" value="{CPN}"/>
				...
				<input type="hidden" name="NEW_ITEM-MANUFACTURER_SKU[{@PrimeLineNo}]" value="{ManufacterSKU}"></input>
				...
				<input type="hidden" name="NEW_ITEM-ZZSAKNR[{@PrimeLineNo}]" value=""/>
			</xsl:for-each>
		</FORM>
	</xsl:template>
</xsl:stylesheet>
