...
FROM yfs_person_info PersonInfo,
     YFS_CUSTOMER C ,
     Yfs_Customer_Addnl_Address Address,
     YFS_CUSTOMER_CONTACT CC 
 
where TRIM(c.root_customer_key) = TRIM(cc.customer_key)
AND Address.CUSTOMER_KEY=C.CUSTOMER_KEY
  AND Address.Person_Info_Key = Personinfo.Person_Info_Key
and TRIM(CC.USER_ID)||'~'||TRIM(C.CUSTOMER_KEY) not in 
  (select TRIM(CA.USER_ID)||'~'||TRIM(CA.CUSTOMER_KEY) from yfs_customer_assignment ca
