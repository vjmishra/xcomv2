...
CREATE OR REPLACE FORCE VIEW XPEDX_LANDING_MIL ("MY_ITEMS_LIST_KEY", "NUMBER_OF_ITEMS", "USERNAME", "CUSTOMER_ID", "LIST_DESC", "LIST_NAME", "CREATEUSERNAME", "SHARE_ADMIN_ONLY", "SHARE_PRIVATE", "MODIFYTS")
AS
...
group by list.my_items_list_key, yu.username, ls.customer_id, LIST.LIST_DESC, LIST.LIST_NAME, LIST.CREATEUSERNAME, LIST.SHARE_ADMIN_ONLY, LIST.SHARE_PRIVATE, list.modifyts ;
...
CREATE OR REPLACE VIEW XPX_ORDER_SEARCH_LIST_VIEW AS
SELECT ORDER_HEADER_KEY,DOCUMENT_TYPE,ENTERPRISE_KEY,ORDER_NO,BUYER_ORGANIZATION_CODE,SELLER_ORGANIZATION_CODE,BILL_TO_KEY,BILL_TO_ID,SHIP_TO_KEY,
SHIP_TO_ID,REQ_SHIP_DATE,ORDER_TYPE,DRAFT_ORDER_FLAG,CUSTOMER_PO_NO,ORDER_NAME,CURRENCY,CUSTOMER_CONTACT_ID,EXTN_WEB_CONF_NUM,EXTN_ORDER_DIVISION,
EXTN_LEGACY_ORDER_NO,EXTN_LEGACY_ORDER_TYPE,EXTN_SHIP_TO_NAME,EXTN_ORDERED_BY_NAME,EXTN_CURRENCY_CODE,EXTN_TOTAL_ORDER_VALUE,EXTN_TOT_ORD_VAL_WITHOUT_TAXES,
EXTN_ORDER_SUB_TOTAL,EXTN_ORDER_STATUS,ORDER_STATUS,STATUS_QUANTITY,HOLD_TYPE,HOLD_STATUS,MODIFYTS,MODIFYUSERID, ORDER_DATE,ADDRESS_LINE1,ADDRESS_LINE2
...
CREATE OR REPLACE VIEW XPX_CUST_VIEW AS
SELECT
...
INNER JOIN YFS_CUSTOMER C ON TRIM(c.root_customer_key) = TRIM(cc.customer_key)
INNER JOIN Yfs_Customer_Addnl_Address Address ON (C.CUSTOMER_KEY = Address.CUSTOMER_KEY) 
INNER JOIN YFS_PERSON_INFO PersonInfo ON  Personinfo.Person_Info_Key = Address.Person_Info_Key
AND TRIM(CC.USER_ID)||'~'||TRIM(C.CUSTOMER_KEY) not in (select TRIM(CA.USER_ID)||'~'||TRIM(CA.CUSTOMER_KEY) from yfs_customer_assignment ca  where CA.user_id is not null)
GROUP BY 
C.Customer_Id ,
