...
SELECT DISTINCT(OH.ORDER_HEADER_KEY),OH.DOCUMENT_TYPE,OH.ENTERPRISE_KEY,OH.ORDER_NO,TRIM(OH.BUYER_ORGANIZATION_CODE) AS BUYER_ORGANIZATION_CODE,OH.SELLER_ORGANIZATION_CODE,OH.BILL_TO_KEY,OH.BILL_TO_ID,OH.SHIP_TO_KEY,
...
,(SELECT MAX(CODE_SHORT_DESCRIPTION) FROM YFS_COMMON_CODE WHERE TRIM(CODE_VALUE)=TRIM(ORS.STATUS))  AS STATUS,OHT.RESOLVER_USER_ID,YOL.ITEM_ID
...
OH.EXTN_ORDER_SUB_TOTAL,TRIM(OH.EXTN_ORDER_STATUS) AS EXTN_ORDER_STATUS,TRIM(ORS.STATUS) AS ORDER_STATUS,ORS.STATUS_QUANTITY,TRIM(OHT.HOLD_TYPE) AS HOLD_TYPE,TRIM(OHT.STATUS) AS HOLD_STATUS,OH.MODIFYTS,OH.MODIFYUSERID,OH.ORDER_DATE AS ORDER_DATE,YPI.ADDRESS_LINE1,YPI.ADDRESS_LINE2
,YPI.ADDRESS_LINE3,YPI.ADDRESS_LINE4,YPI.ADDRESS_LINE5,YPI.ADDRESS_LINE6,YPI.CITY,YPI.STATE,YPI.ZIP_CODE,YPI.COUNTRY ,OH.EXTN_INVOICED_DATE ,OH.EXTN_INVOICE_NO,OH.EXTN_GENERATION_NO
,(SELECT MAX(CODE_SHORT_DESCRIPTION) FROM YFS_COMMON_CODE WHERE TRIM(CODE_VALUE)=TRIM(ORS.STATUS))  AS STATUS,OHT.RESOLVER_USER_ID
FROM YFS_ORDER_HEADER OH 
JOIN YFS_ORDER_RELEASE_STATUS ORS ON ORS.ORDER_HEADER_KEY = OH.ORDER_HEADER_KEY AND ORS.STATUS_QUANTITY=(SELECT MAX(STATUS_QUANTITY) FROM YFS_ORDER_RELEASE_STATUS WHERE ORDER_HEADER_KEY=OH.ORDER_HEADER_KEY)
LEFT JOIN YFS_ORDER_HOLD_TYPE OHT ON OHT.ORDER_HEADER_KEY=OH.ORDER_HEADER_KEY
LEFT JOIN YFS_PERSON_INFO YPI ON YPI.PERSON_INFO_KEY=OH.SHIP_TO_KEY
