#!/bin/ksh
XPDEXHOME=/xpedx
# Note: this scripts is for purging search index from database

PURGEDATABASEHOME=/xpedx/sci_build/PurgeSearchIndexRecords
DATE=`date +%Y-%m-%d`

#location of the log files used for the purge is specified below.

LOGFILE=/xpedx/sci_build/PurgeSearchIndexRecords/logs/searchindexpurgedb-`date +"%Y%m%d-%H%M"`.log

HOST_NAME="$(hostname)"
echo $HOST_NAME


case "$HOST_NAME" in
 zxpappd01) ENVIRONMENT=dev;;
 zxpappd02) ENVIRONMENT=dev;;
 zxpappt01) ENVIRONMENT=staging;; 
 zxpapps01) ENVIRONMENT=prodsupport;; 
 zxpagnt01) ENVIRONMENT=prod;; 
 zxpagnt02) ENVIRONMENT=prod;; 
 zxpaappmc01) ENVIRONMENT=mc;; 
 *);;
esac

        
cd $PURGEDATABASEHOME
echo "about to PURGE database record code using purgesearchindexdb.sh script " >> $LOGFILE
echo "Saving logs to $LOGFILE" 

# execute the purging script using sci_ant
#set -x
#Par1- Delete the search index records from the database
if /xpedx/sterling/Foundation/bin/sci_ant.sh -f purgeDBprocess.xml -v -logfile $LOGFILE purgesearchindex; then
#Par2- Delete the search index records from the physical drive
#find /xpedx/sterling/searchindex/SearchIndex/xpedx/xpedx/MasterCatalog -type d -name "CatalogIndex_*" -mtime +60 -print;
	echo "about to delete old seach index physical folders which are older than 60 days..."
	find /xpedx/sterling/searchindex/SearchIndex/xpedx/xpedx/MasterCatalog -type d -name "CatalogIndex_*" -mtime +60 -exec rm -R {} \;
	#set +x
	exit 0
else
	echo "ERROR/FAILURE! There were errors. Check logs..." 1>&2 >> $LOGFILE
	cat -b $LOGFILE | more
	exit 1
fi

