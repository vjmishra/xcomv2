...
if [[ "$HOST_NAME" = "z$Answer" ]];then
	...
	nohup ./xpedxSearchIndexagentserver.sh xpedxSearchIndexBuild > $LogFile1 2>&1 &
	nohup ./xpedxSearchIndexagentserver.sh xpedxSearchIndexBuild > $LogFile2 2>&1 & 

	echo "tailing log file $LogFile1. You can also check $LogFile2"
	tail -f $LogFile1
