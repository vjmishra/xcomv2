...
#chmod +w *
#cp -p webtrends_$ENVIRONMENT.js webtrends.js
#if [ "$?" != "0" ]; then
...
#chmod +w *
#cp -p webtrends_$ENVIRONMENT.html webtrends.html
#if [ "$?" != "0" ]; then
...
if nohup /xpedx/ant/apache-ant-1.7.1/bin/ant -f build.xml -logfile $LOGFILE; then	
	...
	./sendnotification.sh SWC Success balkhi.mohammad@hp.com,mahmoud.lamriben@hp.com $LOGFILE
	exit 0
else
	echo "ERROR/FAILURE! There were errors in the build. Check logs..." 1>&2  >> $LOGFILE
	./sendnotification.sh SWC Error balkhi.mohammad@hp.com,mahmoud.lamriben@hp.com $LOGFILE
	exit 1
fi
