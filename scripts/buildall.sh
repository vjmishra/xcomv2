...
if nohup ksh -x buildsmcfs.sh; then			 
	...
	if nohup ksh -x buildswc.sh; then			 
		...
		# sucess: then proceed to CDT	
		if nohup ksh -x buildcdt.sh; then			 
			echo "SUCCESS! Completed the CDT build without errors. Continuing with the deployment" >> $LOGFILE			 	 
		else
			echo "ERROR/FAILURE! There were errors in the CDT build. Check logs..." 1>&2 >> $LOGFILE
			exit 1
		fi			
	else
