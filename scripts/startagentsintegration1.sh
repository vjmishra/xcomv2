...
if [[ "$HOST_NAME" = "zxpappint01" ]];then
	...
	nohup ./xpedxagentserver.sh port=1663 XPEDXAgentServer > /xpedx/sterling/Foundation/logs/ReprocessOrders_`date +"%Y%m%d-%H%M"`.log 2>&1 & 
	...
	sleep 10
	nohup ./xpedxStartIntegrationServer.sh port=1664 XPXOrderStatusEmailServer > /xpedx/sterling/Foundation/logs/XPXOrderStatusEmailServer_`date +"%Y%m%d-%H%M"`.log &
	# Jira XBT-73 
	sleep 10 
