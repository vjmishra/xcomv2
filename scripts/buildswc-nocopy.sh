...
LOGFILE=/xpedx/sci_build/WebChannel/logs/SWC-Build-Output-nocopy-`date +"%Y%m%d-%H%M"`.log
...
cp webtrends_prod.js webtrends.js
	#exit 1
fi
...
if [ "$?" != "0" ]; then
	...
	#exit 1
cp webtrends_prod.html webtrends.html
fi
...
if nohup /xpedx/ant/apache-ant-1.7.1/bin/ant -f build.xml -Dbuild.env="$ENVIRONMENT" -logfile $LOGFILE; then	
	...
	echo "SUCCESS! Completed the build without errors. Continuing with the deployment at time" `date +"%Y%m%d-%H%M"` >> $LOGFILE
	...
	cd /home/share/xpadmin/scripts/
	./sendnotification.sh SWC Success mahmoud.lamriben@hp.com,mohammad.balkhi@hp.com,sameer.vasudeva@hp.com $LOGFILE
	echo "Build Completion TimeStamp: " `date` >> $LOGFILE
	
	exit 0
else
	echo "ERROR/FAILURE! There were errors in the build. Check logs..." 1>&2  >> $LOGFILE
	./sendnotification.sh SWC Error mohammad.balkhi@hp.com,sameer.vasudeva@hp.co,mahmoud.lamriben@hp.com $LOGFILE
