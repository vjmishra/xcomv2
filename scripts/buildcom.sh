...
if nohup /xpedx/sterling/Foundation/bin/sci_ant.sh -f Xpedxsmcfsbuild.xml -Dbuild.env="$ENVIRONMENT" build-pca -logfile $LOGFILE; then		
	...
	echo "ERROR/FAILURE! There were errors in the COM build. Check logs..." 1>&2 >> $LOGFILE
	exit 1
fi
