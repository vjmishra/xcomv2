...
if nohup /xpedx/ant/apache-ant-1.7.1/bin/ant -f build.xml -Dbuild.env="$ENVIRONMENT" -logfile $LOGFILE; then	
	...
	./sendnotification.sh SWC Success mahmoud.lamriben@hp.com,mohammad.balkhi@hp.com,manasa-kumar.mohapatro@hp.com,sameer.vasudeva@hp.com $LOGFILE
	echo "Build Completion TimeStamp: " `date` >> $LOGFILE
	
	exit 0
else
	echo "ERROR/FAILURE! There were errors in the build. Check logs..." 1>&2  >> $LOGFILE
	./sendnotification.sh SWC Error mohammad.balkhi@hp.com,manasa-kumar.mohapatro@hp.com,sameer.vasudeva@hp.co,mahmoud.lamriben@hp.com $LOGFILE
