...
public class XPEDXComputeImposition {
	...
	private static double lnWshort ;
	...
	private static double lnrLong ;
	private static int nDown3;
	private static double lnrShort ;
	...
	private static double lnLayoutType ;
	private static final long lnVlong = 300;
	
	public static List<Map<String, String>> calcPage(String sheetSizeW, String sheetSizeH, String trimSizeW, 
			String trimSizeH, String gripperWidth, String colorBarWidth, String sideGuide, String gutter) {
		...
		int nNumout1 = (int)(Math.floor(lnPshort / lnfshort) * Math.floor(lnPlong / lnflong));				
		int nWaste1 = (int)Math.floor(((lnPArea - (lnFarea * nNumout1)) / lnPArea * 100));
		...
		int nNumout2 = (int)(Math.floor(lnPshort / lnflong) * Math.floor(lnPlong / lnfshort)); 
		int nWaste2 = (int)Math.floor(((lnPArea - (lnFarea * nNumout2)) / lnPArea * 100)); 
		...
		if(nRShort3 == 0 || nRShort3 < lnfshort) {
		    ...
		    lncolWaste = (lnPlong - (lnflong * (int)Math.floor((lnPlong / lnflong))));
		  ...
		  int nWaste3 = (int)Math.floor(((lnPArea - (lnFarea * nNumout3)) / lnPArea * 100));
		  ...
		  if(nRShort4 == 0 || nRShort4 < lnfshort) {
		    ...
		    lncolWaste = (lnPshort - (lnflong * (int)Math.floor((lnPshort / lnflong))));
		  ...
		  nrAcross4 = lnrProws;
		  nrDown4 = lnrPCols;
		  int nNumout4 = nNumout2 + ((int)Math.floor((lnrProws)) * (int)Math.floor((lnrPCols)));
		  int nWaste4 = (int)Math.floor(((lnPArea - (lnFarea * nNumout4)) / lnPArea * 100));
		  ...
		  if((int)Math.floor(lnPlong) == 0) {
			  ...
			  nSUnits = (int)Math.floor((lnVlong / lnPlong + lnSGuide + lnGutter));
			  nFlagg = 0;
		  }
		  ...
		  //Width for Gutter on screen
		  double nGutWid = lnGutter * nSUnits;
		  
		  //Width for Side Guide on screen
		  double nSGuidewid = lnSGuide * nSUnits;
		  
		  //Height for Color Bar on screen
		  double nCBarHgt = lnColorb * nSUnits;
		  
		  //Height for Gripper on screen
		  double nGriphgt = lnGripper * nSUnits;
	...
	public static String drawPage(String option) {
		...
		cPassBack = cPassBack + "<TD ALIGN='CENTER'><IMG SRC='/swc/xpedx/images/pls.gif' WIDTH=200 HEIGHT=12 BORDER=0 ALT='Sheet Long Side'></TD>";
		cPassBack = cPassBack + "<TD></TD></TR><TR>";
		cPassBack = cPassBack + "<TD><IMG SRC='/swc/xpedx/images/pss.gif' WIDTH=12 HEIGHT=200 BORDER=0 ALT='Sheet Short Side'></TD>";
		cPassBack = cPassBack + "<TD><TABLE CELLSPACING=0 CELLPADDING=0 BORDER=1><TR>";
		  
		//Add Side Guide if Exists
		if(lnSGuide > 0 ) {
			cPassBack = cPassBack + "<TD BGCOLOR='BLUE' WIDTH=" + (lnSGuide * nSUnits) + "> <IMG SRC='/swc/xpedx/images/bpixel.gif' WIDTH=2 HEIGHT=2 BORDER=0 ALT=''></TD>";
		}
		
		//Add Gutter if Exists
		if(lnGutter > 0) {
		    cPassBack = cPassBack + "<TD BGCOLOR='SILVER' WIDTH=" + (lnGutter * nSUnits) + "> <IMG SRC='/swc/xpedx/images/bpixel.gif' WIDTH=2 HEIGHT=2 BORDER=0 ALT=''></TD>";
		}
		...
		case 1: //Draw Inner Parent Sheet
		    ...
		    cPassBack = cPassBack + "<TABLE CELLSPACING=0 CELLPADDING=0 BORDER=1>";
		       
			   for(int i = 1; i <= nDown1; i++) {
				   cPassBack = cPassBack + "<TR>";
			     for(int j = 1; j<= nAcross1; j++) {
			       cPassBack = cPassBack + "<TD BGCOLOR='FFFFD7' WIDTH=" + (int)Math.floor(nSuTLong);
			       cPassBack = cPassBack + " HEIGHT=" + (int)Math.floor(nSuTshort);
			       cPassBack = cPassBack + "> <IMG SRC='/swc/xpedx/images/bpixel.gif' WIDTH=2 HEIGHT=2 BORDER=0 ALT=''></TD>";
			     }
			     cPassBack = cPassBack + "</TR>";
			   }
		    ...
		    cPassBack = cPassBack + "<TABLE CELLSPACING=0 CELLPADDING=0 BORDER=1>";
		       ...
		       for(int i = 1; i<= nDown2; i++) {
		         ...
		         for(int j = 1; j <= nAcross2; j++) {
		           cPassBack = cPassBack + "<TD BGCOLOR='FFFFD7' WIDTH=" + (int)Math.floor(nSuTshort);
		           cPassBack = cPassBack + " HEIGHT=" + (int)Math.floor(nSuTLong);
		           cPassBack = cPassBack + "> <IMG SRC='/swc/xpedx/images/bpixel.gif' WIDTH=2 HEIGHT=2 BORDER=0 ALT=''></TD>";
		         }
		         cPassBack = cPassBack + "</TR>";
		       }
		...
		case 3 :
			...
			//Draw Inner Parent Sheet
		    if( (int)Math.floor((lnWlong * nSUnits)) - (nAcross3 * nSUnits * nSuTLong) - (nSUnits * nSuTshort * nrAcross3) < 8 ) {
		         nAddin = 8;
		    } else {
		         nAddin = 0;
		    }
		      
		       cPassBack = cPassBack + "<TD VALIGN='TOP' WIDTH=" + (nSuVlong + nAddin);
		       cPassBack = cPassBack + " HEIGHT=" + nSuVShort + " >";
		       cPassBack = cPassBack + "<TABLE CELLSPACING=0 CELLPADDING=0 BORDER=1 ALIGN='LEFT' VALIGN='TOP'>";
		       ...
		       if(nrPieces3 > 0 ) {
		         ...
		         cPassBack = cPassBack + "<TABLE BORDER=1 CELLSPACING=0 CELLPADDING=0 ALIGN='LEFT' VALIGN='TOP'>";
		...
		case 4 :
		  ...
		  if (lnColorb > 0 ) {
		    cPassBack = cPassBack + "<TR><TD BGCOLOR='#00FF00' COLSPAN=" + nColSpan + " HEIGHT=" + (int)Math.floor((lnColorb * nSUnits)) + "> <IMG SRC='/swc/xpedx/images/bpixel.gif' WIDTH=2 HEIGHT=2 BORDER=0 ALT=''></TD></TR>";
		  }

		  //Build Gripper if Exists
		  if( lnGripper > 0 ) {
		    cPassBack = cPassBack + "<TR><TD BGCOLOR='#FF0000' COLSPAN=" + nColSpan + " HEIGHT=" + (int)Math.floor((lnGripper * nSUnits)) + "> <IMG SRC='/swc/xpedx/images/bpixel.gif' WIDTH=2 HEIGHT=2 BORDER=0 ALT=''></TD></TR>";
		  }
		  
		  //Close up Table
		  cPassBack = cPassBack + "</TABLE></TD><TD>&nbsp;</TD></TR>";
		  cPassBack = cPassBack + "<TR><TD>&nbsp;</TD><TD>&nbsp;</TD><TD>&nbsp;</TD></TR></TABLE></TD></TR></TABLE></div>";
